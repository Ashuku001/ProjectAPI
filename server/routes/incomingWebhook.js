...
router.post("/", async function (req, res, next) {
  ...
  console.log("A POST WEBHOOK FOR ", username);

  // const appSecret = await getAppSecret(req).then((value) => {return value})

  const appSettings = await getSettings(username).then((value) => {
                      return value;
                    });

  // console.log("App settings", appSettings, "FOR>>>>>>>", username);
  ...
  if (body.value.hasOwnProperty("messages")) {
    ...
    try {
      let sendReadStatus = messageStatuses.read; //for the message statuses we have a read status
      // console.log('MESSAGE ID TO SEND READ STATUS', body.value.messages[0].id)
      
      sendReadStatus.message_id = body.value.messages[0].id; // add id of the message received to the read status
      const readSent = await sendWhatsAppMessage(sendReadStatus, appSettings); // inform the user that the message has been read through the API
      // console.log("READ STATUS>>>>>>>>>>>", readSent); //
    } catch (error) {
    ...
    // console.log("MESSAGES", body.value.messages);
