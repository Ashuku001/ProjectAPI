		...
		<div>
			{dynamic?.content &&
				...
				<p>{HTMLReactParser(dynamic?.content?.text.replace(/\{\{\d\}\}/g,
					`<span className="text-green-400">${variables[0]}</span>`
				))}</p>
			}
			{dynamic?.inputs?.length !== 0 &&
				<>
					...
					<input type="submit" value="Save" className="bg-green-400 dark:bg-green-700 rounded-sm px-1 py-1 outline-none w-full flex-1 pr-2 cursor-auto" />
				</>
			}
		</div>
	)
