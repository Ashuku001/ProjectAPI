...
import { GetCusInChatInfoDocument } from "../../../../../__gql__/graphql";
import { skipToken, useSuspenseQuery } from "@apollo/client";
type Props = {
    chatId: number
}

function ExistingChat({ chatId }: Props) {

    const { data } = useSuspenseQuery(
        GetCusInChatInfoDocument,
        (chatId && chatId > 0) ? { variables: { chatId: (chatId as number) } } : skipToken
    );
    let customer = data?.chat?.customer
