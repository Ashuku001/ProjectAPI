...
function AddMessage({ chatId }: Props) {
    const [textInput, setTextInput] = useState('')
    const [addMessage, { loading, error, data }] = useMutation(AddMessageDocument)
    console.log("In adding a to an existin chat", textInput)

    const result = data?.addMessage
    ...
    const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
        ...
        addMessage({
            ...
            update(cache, { data: { addMessage } }) {
                const data = cache.readQuery({ query: GetMessagesDocument, variables: { chatId: (chatId as number) } });
                ...
                if (!data?.chat?.messages?.find((msg) => msg?.id === addMessage?.id)) {
                    console.log("in here adding", addMessage, chatId)
